...
class TestsOportundade(TestCase):
    ...
    def test_acesso(self):
        #Acesso a tela
        response = self.client.get('/equipe/membro/historico/')
        self.assertEqual(response.status_code, 200)

        #Retorno aos dados
        response = self.client.get('/equipe/membro/historico/data/')
        self.assertContains(response, '"result": "ok"', status_code=200)

        #Acesso a tela
        response = self.client.get('/equipe/membro/meta/historico/')
        self.assertEqual(response.status_code, 200)

        #Retorno aos dados
        response = self.client.get('/equipe/membro/meta/historico/data/')
        self.assertContains(response, '"result": "ok"', status_code=200)